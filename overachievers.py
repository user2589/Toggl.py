...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="List time records longer than N hours "
                    "(10 by default) in CSV format.")
    ...
    err_writer = csv.DictWriter(sys.stderr,
                                ['user', 'team', 'duration', 'project', 'date',
                                 'rule'])
    ...
    for (monday, sunday) in weeks:
        ...
        for workspace in workspaces:
            ...
            while True:
                report = report_builder.detailed_report(workspace['id'], monday,
                                                        sunday, page)
                ...
                for record in report['data']:
                    ...
                    # missing project
                    # usually None, but want to detect empty string as well
                    if not project:
                    ...
                    # ACTUAL REPORTING
                    super_report[wsname][user][project][week] = \
                        round(super_report[wsname][user][project][week]+hours, 2)
    ...
    report_writer = csv.DictWriter(sys.stdout, ['user', 'team', 'project',
                                                'avg'] + week_names)
    report_writer.writeheader()

    for team, team_records in super_report.items():
        for user, user_records in team_records.items():
            # user_records = super_report[team][user]
            for project, project_records in user_records.items():
                # project_records = super_report[team][user][project]
